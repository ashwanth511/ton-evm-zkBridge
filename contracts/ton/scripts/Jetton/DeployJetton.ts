...
export async function run(provider: NetworkProvider) {
    ...
    let iinitial_supply_Amount: bigint = 10000000000000n;
  

    // Initialize contract with random seed
    const jettons = provider.open(await Jetton.fromInit(content, iinitial_supply_Amount));

    console.log('Contract address:', jettons.address.toString());
    ...
    await provider.waitForDeploy(jettons.address);
